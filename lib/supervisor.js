...
var pre = require('./preprocessors.js');
...
function watchGivenFile (watch, poll_interval) {
    if (isWindows) {
        fs.watch(watch, { persistent: true, interval: poll_interval }, function(event) {
            pre.process(watch, function() { crashWin.call(this, event); });
        });
    }
    else {
        fs.watchFile(watch, { persistent: true, interval: poll_interval }, function(oldStat, newStat) {
            pre.process(watch, function() { crashOther.call(this, oldStat, newStat); });
        });
    }
}
